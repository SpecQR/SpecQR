...
| GS1 validation API | Tested | `getSupportedGs1Ais()`、`getGs1AiInfo(ai)`、`validateGs1Elements()`、`validateGs1ElementString()`、GS1 detail error code、packed package smoke、TypeScript surface を確認しています。Digital Link 専用の `validateGs1DigitalLink()` は未公開ですが、v2.2.0 proposal は [GS1 Digital Link Validation v2.2 Design](./gs1-digital-link-validation-v2.2.md) に固定しています。 |
| GS1 Digital Link | Partial | `createGs1DigitalLink()` で supported AI から通常 URL QR 用の Digital Link URI を生成し、`parseGs1DigitalLink()` で URI を element data に戻せます。現在の output は deterministic builder であり、default path/query placement と invalid path placement rejection は dictionary role metadata で確認しています。v2.2.0 では non-throwing validation と SpecQR deterministic normalization を候補にしますが、resolver、compression、full canonicalizer は未実装です。 |
...
| GS1 Digital Link helper | Partial / tested | `createGs1DigitalLink()` と `parseGs1DigitalLink()` は minimal create/parse + role metadata として実装済みです。`validateGs1DigitalLink()` / `normalizeGs1DigitalLink()` は [GS1 Digital Link Validation v2.2 Design](./gs1-digital-link-validation-v2.2.md) に proposal として固定し、resolver、compression、full canonicalizer は引き続き deferred として残しています。 |
