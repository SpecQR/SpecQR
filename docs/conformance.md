...
| Structured Append parity helper | Tested | low-level `structuredAppend` 利用者向けの `calculateStructuredAppendParity(input)` は実装済みです。string は UTF-8、binary input は original bytes、`ArrayBufferView` は offset / length を尊重します。Manual segments 専用 helper はまだ runtime API ではなく、`generateSegmentsStructuredAppend()` と同じ canonical logical bytes を使う設計を docs-only で固定しています。 |
