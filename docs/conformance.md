...
| Structured Append parity helper | Not supported / proposed | `calculateStructuredAppendParity(input)` は v2.3.0 の docs-only proposal として [Structured Append Parity Helper v2.3 Design](./structured-append-parity-v2.3.md) に固定済みです。現行 runtime、package exports、TypeScript declarations にはまだ存在しません。 |
...
| Structured Append parity helper | Proposed | low-level `structuredAppend` 利用者向けの `calculateStructuredAppendParity(input)` は v2.3.0 の first polish candidate です。string は UTF-8、binary input は original bytes、`ArrayBufferView` は offset / length を尊重し、manual segments 専用 helper は後続候補にします。 |
