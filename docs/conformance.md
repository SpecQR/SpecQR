...
| Structured Append scanning / merge | Not supported | SpecQR は decoder や scanner integration を提供しません。decoder が自動 merge すること、または `index` / `total` / `parity` metadata を露出することは保証しません。読み取り workflow と将来の `mergeStructuredAppendParts(parts, options?)` 案は [Structured Append Scanning Workflow](./structured-append-scanning-v2.md) に、metadata-returning decoder 候補と optional validation 方針は [Structured Append Decoder Metadata Validation](./structured-append-decoder-validation-v2.md) に docs-only で固定しています。 |
