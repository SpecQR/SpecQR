...
- v1 で未対応の Micro QR、rMQR、Structured Append automatic splitting / parity helper、logo overlay、styled modules、GS1 full AI catalog、GS1 Digital Link resolver / compression / full canonicalizer が docs に明記されている。
...
- npm install:
  ...
  npm install specqr
  ```
- 主な対応範囲: QR Code Model 2 Version 1-40、L/M/Q/H、Numeric / Alphanumeric / Byte / Kanji、ECI、GS1/FNC1 first position、FNC1 second position、Structured Append low-level header、SVG/PNG/canvas/Node/browser helpers。
- 検証: golden conformance、jsQR decoder validation、macOS Vision validation、Nayuki reference comparison。
- 非対応: Micro QR、rMQR、Structured Append automatic splitting / parity helper、logo overlay、styled modules、GS1 full AI catalog、GS1 Digital Link resolver / compression / full canonicalizer。
