...
- Micro QR、rMQR、Structured Append public parity helper runtime implementation / QR decoder / scanner integration、logo overlay、styled modules、GS1 full AI catalog、GS1 Digital Link resolver / compression / full canonicalizer が docs に非スコープとして明記されている。Structured Append parity helper の v2.3.0 docs-only proposal は [Structured Append Parity Helper v2.3 Design](./structured-append-parity-v2.3.md) に分ける。
...
- 非対応: Micro QR、rMQR、Structured Append public parity helper runtime implementation / QR decoder / scanner integration、logo overlay、styled modules、GS1 full AI catalog、GS1 Digital Link resolver / compression / full canonicalizer。Structured Append parity helper の v2.3.0 docs-only proposal は別設計文書として扱う。
