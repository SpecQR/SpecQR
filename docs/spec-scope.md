...
現在の main branch は `3.0.0-rc.1` candidate で、v2 stable scope の GS1 strict
...
diagnostics contract だけです。RC は未公開であり、別系統の QR family や装飾機能は
core へ混ぜません。

RC 1 は release freeze 状態です。Structured Append diagnostics contract 以外の
runtime、型、public export の変更は追加しません。unknown-option rejection、GS1
...
3.0.0-rc.1 では一度作った canonical tarball を Node 18 / 20 / 22 / 24 へ
