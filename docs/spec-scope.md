...
v2 準備として、raw GS1 element string を internal validator で parse / validate できる基盤を持っています。この validator は parentheses を含まない payload を対象にし、fixed-length AI は dictionary の exact length で読み、variable-length AI は ASCII GS separator があれば次 element へ進み、separator なしなら final element として扱います。variable-length AI の後に別 AI が続く場合は separator が必要です。raw element string は括弧がないため、final variable-length value の末尾が supported fixed-length AI に見える曖昧ケースは、推測で分割せず missing separator として reject します。この validator は `generate(input, { gs1: true })` と diagnostics に内部統合していますが、現時点では public API ではなく、`parseGs1HumanReadable()` と `createGs1ElementString()` の v1 behavior は維持します。
