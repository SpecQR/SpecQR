...
Decoder validation should remain secondary. Scanner APIs differ in how they expose Structured Append set metadata, so conformance should rely on header bits, parity, matrix / codeword golden fixtures, diagnostics, and `mergeStructuredAppendParts()` unit coverage. 読み取り側 workflow と merge helper の境界は [Structured Append Scanning Workflow](./structured-append-scanning-v2.md) に分けています。
