...
- GS1 Digital Link conversion: `createGs1DigitalLink()` の URL construction、baseUrl validation、path/query placement、invalid GS1 value rejection を確認する。次フェーズでは `parseGs1DigitalLink()` の相互変換を追加し、FNC1 first の raw GS1 element string と URL-based QR を混同しないことを固定する。詳細な API proposal と validation policy は [GS1 Digital Link v2 Design](./gs1-digital-link-v2.md) を参照する。
