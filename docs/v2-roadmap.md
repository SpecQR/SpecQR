...
23. **Planning / diagnostics API implementation**: docs に固定した設計に従い、runtime export、`QRCode` static method、TypeScript declarations、unit tests、packed smoke を追加する。完了済み。
...
- 2026-05-30: v2.4.0 release package prepared. `package.json` / `package-lock.json` の version を `2.4.0` に揃え、CHANGELOG、README、API / release docs を Planning API stable publish 前提に整理しました。npm publish、GitHub Release、GitHub Pages deploy、`v2.4.0` tag 作成は最終承認まで行いません。
- 2026-07-31: Structured Append ZXing Java metadata lane を required verification へ昇格しました。Maven Wrapper 3.3.4 / Maven 3.9.16 / ZXing core + javase 3.5.4 を独立 harness へ固定し、packed package が生成する 10 fixtures / 44 symbols の sequence/parity metadata と 7 text payload merge を Ubuntu 専用 CI job で確認します。Binary 3 cases は理由付き metadata-only です。Runtime dependency、public API、package exports は変更していません。

## v2.4.0 Planning / Diagnostics API Scope

v2.4.0 は、新しい QR control mode や renderer feature ではなく、既存 core planner と diagnostics を public planning surface として安全に使えるようにする API release として扱います。この section の API は実装済みで、root export と `QRCode` static method の両方から利用できます。
...
Structured Append や FNC1 second は decoder によって露出方法が異なる可能性があるため、decoder validation だけを唯一の根拠にしません。control segment bit length、matrix / codeword golden fixtures、diagnostics、negative tests を組み合わせて release gate とします。Structured Append の読み取り後 merge は `mergeStructuredAppendParts()` の unit/packed smoke に加え、固定版 ZXing Java 3.5.4 の required dedicated job で sequence/parity metadata と信頼できる text payload を確認します。全 scanner 互換、arbitrary binary string round-trip、scanner 側自動 merge は non-claims です。詳細は [Structured Append ZXing Java Verification](./structured-append-zxing-java.md) を参照してください。FNC1 second と Structured Append low-level header の基本 coverage は実装済みです。
